...
@interface ZZCreateCaseController ()<ZCActionSheetViewDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate,ZZCreateCaseDeleate>{
...
@implementation ZZCreateCaseController
...
- (void)viewDidLoad {
    ...
    if(_editModel){
        _isEdit = YES;
        [self.menuTitleButton setTitle:@"编辑病例" forState:UIControlStateNormal];
        
    }else{
        _isEdit = NO;
        _editModel = [ZZCaseModel new];
        
        [self.menuTitleButton setTitle:@"创建新病例" forState:UIControlStateNormal];
    }
...
-(void)confirmHZByCaseId:(NSString *) caseId{
    ...
    // 问卷类型 1普通 2运动
    [dict setObject:convertToString(@"1") forKey:@"type"];
    //
