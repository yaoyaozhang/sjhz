...
@objc protocol MHAVPlayerSDKDelegate: NSObjectProtocol {
    ...
    @objc optional func mhShareListener()
}
...
class MHAVPlayerSDK: UIView {
    ...
    fileprivate lazy var topMenu: MHTopMenu = {
        ...
        topMenu.backgroundColor = UIColor.clear //UIColor(red: 50.0/255.0, green: 50.0/255.0, blue: 50.0/255.0, alpha: 1.0)
        topMenu.isHidden = false
        return topMenu
    }()
...
//        bottomMenu.backgroundColor = UIColor(colorLiteralRed: 50.0/255.0, green: 50.0/255.0, blue: 50.0/255.0, alpha: 1.0)
        bottomMenu.backgroundColor = UIColor.clear
    ...
    public func mhPausePlayer() {
        mhPlayer.mhPause()
    }
    
    
    public func mhPlayPlayer() {
        mhPlayer.mhPlaye()
    }
    ...
    fileprivate func mhAVPlayerBLock() {
        ...
        MHClosure.mhDirectionChangeClosure = {[weak self] (origent) in
            ...
            if sself.mhAutoOrient == true {
                if origent == UIDeviceOrientation.landscapeLeft || origent == UIDeviceOrientation.landscapeRight {
...
extension MHAVPlayerSDK {
    ...
    fileprivate func mhTopMenuBlock() {
        ...
        topMenu.mhTopShareListener = { [weak self] in
            guard let sself = self else {return}
            //返回
            if sself.bottomMenu.mhFull == true {
                UIDevice.setOrientation(false)
                ...
                if (sself.MHAVPlayerSDKDelegate?.responds(to: NSSelectorFromString("mhShareListener")))! {
                    sself.MHAVPlayerSDKDelegate?.mhShareListener!()
                }
            }
        }
    }
...
extension MHAVPlayerSDK {
    ...
    override func layoutSubviews() {
        ...
        topMenu.frame = CGRect(x: 0, y: backView.top+UIApplication.shared.statusBarFrame.height, width: backView.width, height: 40)
