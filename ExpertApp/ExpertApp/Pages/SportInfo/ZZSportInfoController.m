#import "ZZSportInfoController.h"

//
//  ZZSportInfoController.m
//  ExpertApp
//
...
@interface ZZSportInfoController ()<ZCActionSheetViewDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate,ZZCreateCaseDeleate>{
...
@implementation ZZSportInfoController
...
-(void)getDetail{
    if(convertToString(_pCaseId).length>0){
        ...
        [ZZRequsetInterface post:API_SearchCaseDetail param:dict timeOut:HttpGetTimeOut start:^{
            ...
            _editModel = [[ZZSportCaseEntity alloc] initWithMyDict:dict[@"retData"]];
            [self refreshViewData:YES];
