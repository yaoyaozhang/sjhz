...
+ (UIImage *)zc_animatedGIFWithData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self zc_frameDurationAtIndex:i source:source];
...
+ (UIImage *)zc_imageWithData:(NSData *)data {
    ...
    UIImage *image;
    NSString *imageContentType = [self zc_contentTypeForImageData:data];
    if ([imageContentType isEqualToString:@"image/gif"]) {
        image = [self zc_animatedGIFWithData:data];
    }
    //#ifdef SD_WEBP
    ...
    //#endif
        ...
        //        }
        image = [self fastImageWithData:data];
    }
...
+ (float)zc_frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
...
+ (UIImage *)zc_animatedGIFNamed:(NSString *)name {
    ...
    if (scale > 1.0f) {
        ...
        if (data) {
            return [self zc_animatedGIFWithData:data];
        }
...
+ (UIImage *)zc_animatedImageByScalingAndCroppingToSize:(CGSize)size with:(UIImage *)pimage{
