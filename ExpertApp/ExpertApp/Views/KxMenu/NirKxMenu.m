                   ...
                   action:(SEL) action
{
    return [[KxMenuItem alloc] init:title
                              image:image
                             target:target
                             action:action tag:0 objTag:nil];
}
+ (instancetype) menuItem:(NSString *) title
                    image:(UIImage *) image
                      tag:(int)intTag
                   ...
                   action:(SEL)action
{
    return [[KxMenuItem alloc] init:title
                             ...
                             target:target
                             action:action
                                tag:intTag objTag:nil];
}
+(instancetype) menuItem:(NSString *)title image:(UIImage *)image tag:(int)intTag objTag:(id)objTag target:(id)target action:(SEL)action{
    return [[KxMenuItem alloc] init:title
                              image:image
                             target:target
                             action:action
                                tag:intTag objTag:objTag];
}
...
- (UIView *) mkContentView:(CGRect ) sRect
{
    for (UIView *v in self.subviews) {
    ...
    const CGFloat kMinMenuItemWidth = sRect.size.width;
    //配置：左右边距
