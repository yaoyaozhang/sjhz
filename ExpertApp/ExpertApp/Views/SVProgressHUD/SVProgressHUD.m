...
static const CGFloat SVProgressHUDAlpha=0.85f;
...
+ (void)showImage:(UIImage*)image status:(NSString*)status{
    ...
    [[self sharedView] showImage:image status:status duration:round(displayInterval*10)/10];
}
...
- (instancetype)initWithFrame:(CGRect)frame{
    if((self = [super initWithFrame:frame])){
        ...
        self.alpha = SVProgressHUDAlpha;
        self.activityCount = 0;
        
        _defaultMaskType = SVProgressHUDMaskTypeNone;
        _defaultStyle = SVProgressHUDStyleDark;
        _defaultAnimationType = SVProgressHUDAnimationTypeNative;
        ...
        UIImage* infoImage = [UIImage imageWithContentsOfFile:[imageBundle pathForResource:@"pop_warning_attention" ofType:@"png"]];
        UIImage* successImage = [UIImage imageWithContentsOfFile:[imageBundle pathForResource:@"pop_warning_right" ofType:@"png"]];
                         ...
                         animations:^{
                             ...
                             if(strongSelf){
                                 ...
                                 if(strongSelf.isClear){ // handle iOS 7 and 8 UIToolbar which not answers well to hierarchy opacity change
                                     strongSelf.hudView.alpha = SVProgressHUDAlpha;
                                 } else{
                                     strongSelf.alpha = SVProgressHUDAlpha;
                                 }
                             }
                         }
...
- (UIImageView*)imageView{
    if(!_imageView){
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 40.0f, 37.0f)];
