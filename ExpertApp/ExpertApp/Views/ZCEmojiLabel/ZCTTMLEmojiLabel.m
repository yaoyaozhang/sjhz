...
const CGFloat kZCLineSpacing = 4.0;
const CGFloat kZCAscentDescentScale = 0.25; //在这里的话无意义，高度的结局都是和宽度一样

const CGFloat kZCEmojiWidthRatioWithLineHeight = 1.15;//和字体高度的比例

const CGFloat kZCEmojiOriginYOffsetRatioWithLineHeight = 0.10; //表情绘制的y坐标矫正值，和字体高度的比例，越大越往下
NSString *const kZCCustomGlyphAttributeImageName = @"CustomGlyphAttributeImageName";

#define kEmojiReplaceCharacter @"\uFFFC"

#define kURLActionCount 5
NSString * const kZCURLActions[] = {@"url->",@"email->",@"phoneNumber->",@"at->",@"poundSign->"};

/**
 *  搞个管理器，否则自定义plist的话，每个label都会有个副本很操蛋
 */
@interface ZCTTMLEmojiLabelRegexPlistManager : NSObject
...
@implementation ZCTTMLEmojiLabelRegexPlistManager

+ (instancetype)sharedInstance {
    static ZCTTMLEmojiLabelRegexPlistManager *_sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
@interface TTTAttributedLabel(ZCMLEmojiLabel)
...
@implementation ZCTTMLEmojiLabel
...
//            }
//            emojiDictionary = [[ConfigManager getInstance] allExpressionDict];
//        }
...
- (void)commonInit {
    ...
    self.lineSpacing = kZCLineSpacing; //默认行间距
           ...
           context:(CGContextRef)c
{
    ...
    CGFloat emojiWith = self.font.lineHeight*kZCEmojiWidthRatioWithLineHeight;
    CGFloat emojiOriginYOffset = self.font.lineHeight*kZCEmojiOriginYOffsetRatioWithLineHeight;
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns(line)) {
            ...
            //判断是否有图像，如果有就绘制上去
            NSString *imageName = attributes[kZCCustomGlyphAttributeImageName];
            if (imageName) {
...
//                                                           range:NSMakeRange(0, [emojiText length])];
    ...
    for (NSTextCheckingResult *result in emojis) {
        ...
        NSDictionary *emojiDict = self.customEmojiRegularExpression?self.customEmojiDictionary:[ZCTTMLEmojiLabel emojiDictionary];
        ...
        if (imageName) {
            ...
            metrics->ascent = 1/(1+kZCAscentDescentScale)*metrics->width;
            metrics->descent = metrics->ascent*kZCAscentDescentScale;
            ...
            // 设置自定义属性，绘制的时候需要用到
            [attrStr addAttribute:kZCCustomGlyphAttributeImageName
                            value:imageName
                            range:__range];
        } else {
...
- (void)setText:(id)text
{
    ...
    for (NSUInteger i=0; i<maxIndex; i++) {
        ...
        NSString *urlAction = kZCURLActions[i];
...
-(NSMutableDictionary *) getTextADict:(NSString *) searchText{
    ...
    for (NSTextCheckingResult *match in arrayOfAllMatches)
    {
        ...
        NSString *replaceText = [substringForMatch stringByReplacingOccurrencesOfString:@" " withString:@""];
...
- (void)setCustomEmojiRegex:(NSString *)customEmojiRegex
{
    ...
    if (customEmojiRegex&&customEmojiRegex.length>0) {
        self.customEmojiRegularExpression = [[ZCTTMLEmojiLabelRegexPlistManager sharedInstance]regularExpressionForRegex:customEmojiRegex];
    }else{
...
- (void)setCustomEmojiPlistName:(NSString *)customEmojiPlistName
{
    ...
    if (customEmojiPlistName&&customEmojiPlistName.length>0) {
        self.customEmojiDictionary = [[ZCTTMLEmojiLabelRegexPlistManager sharedInstance]emojiDictForKey:customEmojiPlistName];
    }else{
    ...
    if(self.delegate&&[self.delegate respondsToSelector:@selector(ZCMLEmojiLabel:didSelectLink:withType:)]){
        if (self.activeLink&&self.activeLink.result.resultType==NSTextCheckingTypeCorrection) {
            ...
            for (NSUInteger i=0; i<kURLActionCount; i++) {
                if ([result.replacementString hasPrefix:kZCURLActions[i]]) {
                    NSString *content = [result.replacementString substringFromIndex:kZCURLActions[i].length];
                    //type的数组和i刚好对应
